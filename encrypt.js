...
let handler = async (m, { Ditss, example, mime, isCreator,isPremium }) => {
...
await JsConfuser.obfuscate(await fs.readFileSync(`./library/database/sampah/${filename}`).toString(), {
  ...
  await Ditss.sendMessage(m.chat, {document: fs.readFileSync(`./library/database/sampah/${filename}`), mimetype: "application/javascript", fileName: filename, caption: "Encrypt file sukses ✅"}, {quoted: m})
